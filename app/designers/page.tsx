...
export default function DesignersPage() {
  return (
    <div className="px-4 py-14 md:px-8">
      <h1 className="mb-14 font-sans text-4xl font-medium uppercase tracking-tight md:text-6xl">Designers</h1>
      <ul>
        {designers.map((d) => {
          ...
          return (
            <li key={d.slug} className="border-b border-black/10">
              <Link href={`/designers/${d.slug}`} className="group flex items-baseline justify-between gap-4 py-6">
                <span className="font-sans text-4xl uppercase leading-none tracking-tight transition-colors group-hover:text-black/60 sm:text-6xl md:text-7xl">
                  {d.name}
                </span>
